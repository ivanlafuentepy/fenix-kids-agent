...
| `JUEGO_ESTACIONES` | ⚠️ Cambiada a `quincho` en Railway (24/07, piloto de 1 sola estación) — default original `ninja,arbol,basket,quincho,muelle` | Estaciones del circuito NFC. Los `id` deben matchear el `estacion_id` de los ESP32 y del `mapa.html`. Revertir a las 5 cuando estén armadas ninja/arbol/basket/muelle (pendiente #282). |
...
| 283 | JUEGO — revertir `JUEGO_ESTACIONES` en Railway a `ninja,arbol,basket,quincho,muelle` (+ restart) cuando estén armadas las otras 3 estaciones — hoy quedó en `quincho` solo, para el piloto de 1 estación del 25/07 | ⏳ Pendiente (post-piloto) |
