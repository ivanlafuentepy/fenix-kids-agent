...
**Plan mensual (4 sábados):** (cuota, sin matrícula)
| Hijos | Precio |
|---|---|
| 1 hijo | 240.000 Gs |
...
Lógica: +100.000 por cada hijo extra.
...
| `ELEVENLABS_API_KEY` | ⏳ Futuro (juego) | Voz del Guardián (TTS George) para el Modo TV / tótem. Solo se usa en el script que pre-genera MP3 de alumnos (`scripts/generar_voces_alumnos.py`, aún no creado). Cuenta free de Iván. NO la usa el agente en runtime. |
| `JUEGO_API_KEY` | ⏳ Futuro (juego) | Header `X-JUEGO-KEY` para los endpoints del juego (`/juego/checkin-face`, `/juego/evento`) que ejecutará Opus según `mundo-fenix/SPEC-TOTEM-Y-PROFE.md`. Aún no existen. |
