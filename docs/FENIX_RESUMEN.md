...
| 302 | `crear_o_actualizar_tutor` (`airtable_client.py:611`) busca idempotencia solo por `{TELEFONO LIMPIO}` exacto → si alguien se inscribe con un número que vive en el `TELEFONO2` de otra fila, crea **duplicado**. Hoy solo Gaudi tiene TELEFONO2 (riesgo bajo) | ⏳ Pendiente |
