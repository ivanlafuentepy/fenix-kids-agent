...
| 322 | ~~**AFICHES NUEVOS del campus de 2 días**~~ — Iván mandó el PNG el 17/08, verificado con Read (300/450, sábado-domingo, merienda niño+padres) y publicado en las 3 rutas (`53f2055` agente, `c3ed9a3` web); peso confirmado en prod en ambos lados. Sigue siendo UNA sola pieza para precios y horarios (#314 sigue abierto) | ✅ Hecho 17/08 |
...
| 326 | Los guiones de `marketing/docs/` (3 menciones) tienen el **link viejo de maps** (ficha de La Casona). No se tocaron por scope — decidir si se actualizan al de la ficha de Fenix | ⏳ Decisión de Iván |
| 328 | **Verificar el fix del menú end-to-end en prod**: `modo padre` desde el WhatsApp de Iván → tocar *Info y precios* → escribir una pregunta libre → tiene que responder Aurora, no el recordatorio de botones (`e3e3500` deployado, falta la prueba con un mensaje real) | ⏳ Pendiente |
| 329 | **Lead 595982862766 sin respuesta** desde el 18/08 13:47 (preguntó si el campus es todos los meses). El fix no le contesta solo — necesita el OK de Iván para enviarle la respuesta | ⏳ Decisión de Iván |
| 330 | **Mundo Fenix — `estacion.html`**: pantalla por estación que pollea `GET /juego/eventos` y filtra por `estacion_id` (nombre + guardián + "¡BIEN HECHO!", con el auto-reload del kiosk). Paso 1 acordado: probarla en un **celular viejo** en la estación del gym antes de comprar una sola TV | ⏳ Pendiente |
| 327 | **El preview del link de ubicación lo decide Google**: la imagen sale de la foto de portada de la ficha *Fenix Kids Academy* en Google Business. Si Iván quiere que se vea el LOGO, hay que ponerlo ahí (el código no lo controla). Alternativa nuestra: que el agente mande el logo como imagen junto a la ubicación | ⏳ Operacional / decisión |
