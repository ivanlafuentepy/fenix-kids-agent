...
| 223 | EJE B / B2 pendiente — migrar lecturas CORE a TUTORES, deploy aislado c/u + verificación en vivo: `buscar_familia_por_telefono` (airtable_client:522), `buscar_familia_por_nombre` (:462), `familia_es_activa` (:553), webhook "quién escribe" `_build_contexto_aurora` (main.py:2008-2072). NO migrar resumenes.py:306 ni APIs web main.py:692/777 (código a rehacer + mapas masivos) | ⏳ Pendiente (próxima sesión) |
