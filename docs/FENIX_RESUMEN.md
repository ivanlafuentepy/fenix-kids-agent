...
| 229 | EJE B / B2 — DECISIÓN: lo que corre en hot-path (nombre del topic Telegram main.py:2841; Grupo 2 = `buscar_familia_por_telefono`/`por_nombre`/`familia_es_activa`/`obtener_familias_inscriptas`) NO se migra con el helper (sumaría GETs). Se resolverá con **lookups/rollups en Airtable** (datos de tutores pre-cargados en el registro de familia, 0 fetch extra). NO migrar resumenes.py:306 ni APIs web main.py:692/777 (código a rehacer) | ⏳ Pendiente (próxima sesión — diseñar lookups) |
