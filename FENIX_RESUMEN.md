...
| 2026-05-12 (sesión 4) | **Optimización tokens -41% + intercepción objeciones + CHECKLIST.md (3 commits).** (1) **Prompt Ivan comprimido**: de ~3,440 a ~2,020 tokens (-41%). FASE 1 literal eliminada (interceptada por código), objeciones fáciles movidas a código, fases 3-5 comprimidas, frase ancla reducida. (2) **6 objeciones interceptadas por código**: duración, qué llevar, devolución, efectivo/medios de pago, "ya transferí" sin foto, alias. Detectores regex + respuestas fijas, 0 tokens Claude. (3) **"Lo voy a pensar" eliminada del prompt**: se maneja con followup al día siguiente. (4) **FASE 1 interceptada por código**: mensaje de apertura fijo para leads nuevos sin llamar a Claude. Fix condición: `len(historial) <= 1` por early save (el mensaje del usuario ya está en historial). (5) **Incidente**: afirmé que FASE 1 ya estaba cubierta por código sin verificar → lead real recibió mensaje genérico improvisado. (6) **CHECKLIST.md**: checklist obligatorio antes de cambios en prompts/flujo/deploy. Pasos: leer, grep dependencias, verificar cobertura con evidencia, simular impacto, presentar a Ivan. |
