...
agent/
  ...
  brain.py          — Llama a Claude API con tool_use (retrocompatible). Carga prompts según agente.
  memory.py         — Historial de conversaciones + estado + pagos persistentes + dedup
  ab_test.py        — Estado por conversación: agente, modo, familia_id, estado_json (flags persistentes)
  tool_definitions.py — Schemas de tools para Claude (reagendar_clase, confirmar_reserva)
  tool_executor.py  — Dispatcher: tool_name → función Python
  tools/            — Funciones de negocio extraídas de main.py
    detectores.py   — 10 detectores de intención (padre_pregunta_precios, etc.)
    info.py         — Respuestas FAQ (precios, ubicación, etc.) — referencia para Tool Use futuro
    reservas.py     — reagendar_clase (actualiza PRUEBA FENIX en Airtable)
...
| 191 | Página FU pruebas con wa.me personalizados por edad + endpoint /fu/{archivo} | ✅ Hecho |
| 192 | Endpoint /test-envio/{tel} para enviar mensajes desde Railway | ✅ Hecho |
| 193 | Skill /fusabado — genera página FU automatizada desde Airtable | ✅ Hecho |
| 194 | Skill /endpoint — analiza conversación de producción | ✅ Hecho |
| 195 | "modo padre" = reset completo + activar flujo padre (reemplaza holayosoyfenix) | ✅ Hecho |
| 196 | FU excluye CONVERSION=INSCRIPTO automáticamente | ✅ Hecho |
| 197 | Carpeta .claude/skills/ para skills complejas (proxy desde commands/) | ✅ Hecho |
...
| 2026-05-12 (sesión 4) | **Optimización tokens -41% + intercepción objeciones + CHECKLIST.md (4 commits).** (1) **Prompt Ivan comprimido**: de ~3,440 a ~2,020 tokens (-41%). FASE 1 literal eliminada (interceptada por código), objeciones fáciles movidas a código, fases 3-5 comprimidas, frase ancla reducida. (2) **6 objeciones interceptadas por código**: duración, qué llevar, devolución, efectivo/medios de pago, "ya transferí" sin foto, alias. Detectores regex + respuestas fijas, 0 tokens Claude. (3) **"Lo voy a pensar" eliminada del prompt**: se maneja con followup al día siguiente. (4) **FASE 1 interceptada por código**: mensaje de apertura fijo para leads nuevos sin llamar a Claude. Fix condición: `len(historial) <= 1` por early save (el mensaje del usuario ya está en historial). (5) **Incidente**: afirmé que FASE 1 ya estaba cubierta por código sin verificar → lead real recibió mensaje genérico improvisado. (6) **CHECKLIST.md**: checklist obligatorio antes de cambios en prompts/flujo/deploy. (7) **Fix formulario post-pago**: fechas en texto ("22 Dic 2021") no se detectaban — el detector solo buscaba "/" o "-". Ahora detecta meses en español. Lead Nancy Segovia/Sebastián García cargado manualmente. |
| 2026-05-13 | **Sesión de FU masivo + 21 commits (precios/formulario/prompt/resumen/llamada).**  (1) **FU Video masivo**: 54 leads CONSULTA/CONTACTADO con ventana 24h, video + "Regalale a tu hijo un sábado que recordará...". 54/54 exitosos, 0 fallidos. Script `scripts/fu_video_13mayo.py`. (2) **Campo ULTIMO MENSAJE en Airtable**: tracking automático de ventana 24h. Cada mensaje entrante actualiza el campo. Ya no se necesita consultar PostgreSQL para filtrar ventana. (3) **Fix monto 90mil→120mil**: `monto_prueba_por_hijos` no detectaba formato "Papá + 2 hijos: 120.000 Gs" junto a datos bancarios. Caía al fallback 90mil. Nuevo patrón: detecta cualquier monto en mensaje con Banco/Cta/Alias. (4) **Fix fecha reserva ISO**: "16 de mayo" no es fecha ISO → Airtable la rechazaba silenciosamente. Ahora convierte "16 de mayo" → "2026-05-16". (5) **Fix RUC = fecha falsa**: guión en RUC "3971573-6" disparaba formulario completo antes de recibir datos del hijo. Ahora guiones solo cuentan como fecha con formato dd-mm-yyyy. (6) **Fix precios TOTAL**: el agente sumaba "90 Thiago + 120 Sofía = 210mil". Prompt reescrito: "2 hijos = 120mil TOTAL. NUNCA sumar. NUNCA desglosar." (7) **Fix mamá adicional**: agente inventaba cobro extra por la mamá. Regla explícita: padres entran GRATIS siempre. (8) **Fix clase prueba ≠ pase suelto**: agente decía "90mil por sábado, venís cuando quieras". Ahora: clase de PRUEBA, después plan. (9) **Formulario post-pago**: formato exacto con emojis diferenciados (👤 padre, 👦 hijo, 🎂 fecha nac). NUNCA "nene/a". (10) **Prohibido pibe/nene/chico/campeón**: siempre "tu hijo" o el nombre. (11) **Conversación no pitch**: no apurar, responder lo que preguntan, no redirigir al sábado, no repetir preguntas. (12) **Planes solo por afiche**: NUNCA dar precios de planes, solo "te paso un afiche". (13) **Notif Telegram pago**: solo tipo+monto+link (sin "Padre: Lead, Hijo: no mencionó"). Formulario: datos completos con hijos y fechas. (14) **Aurora totales sin nombres**: disponibilidad muestra solo "7 agendados" por horario, incluye prueba, sin nombres de otros niños. (15) **GUIA FOLLOWUP FENIX.md** + BITACORA movidas a Obsidian Vault. 4 registros corregidos manualmente en Airtable (Juan/Marcelo/Mateo, Milagros/Joaquín, Leticia/Jeremías/Matías, Patricia/Marco). (16) **Resumen anuncios mejorado**: leads totales por día, % conversión, media por día, espacio entre días, mayo desde el 3 (anuncios no existían 1-2). (17) **Comandos Telegram en menú**: /silenciar, /reactivar, /aprobado, /rechazado, /fenix, /registro, /agenda. (18) **Resumen diario automático 8AM PY**: reemplaza keepalive, envía resumen anuncios + reservas al admin. (19) **Fix detector llamada**: "quiero hablarlo con mi esposa" ya no activa alerta de llamada (regex restringido a hablar con vos/usted/Ivan). (20) **Obsidian**: conversaciones 11/05 y 12/05 vinculadas con up::, MOC con links individuales. |
| 2026-05-15 | **Nueva estructura precios + promo 4P + regla de silencio + fix regex fecha (4 commits).** (1) **Precios prueba actualizados**: 90k/120k/150k normal + promo hoy 100k/150k/180k (2 sábados si transfiere ese día). Sin matrícula, sin vencimiento. (2) **Paquetes nuevos**: 5 clases 350k, 12 clases 750k. Descuentos familiares: paq 5 (2do hijo 30%, 3ro 50%), paq 12 (2do hijo 40%, 3ro gratis 3x2, total 3 hijos 1.200.000). (3) **Afiches nuevos**: afiche_fenix.png (precios+paquetes+promo) + afiche_hermanos.png (plan hermanos). (4) **Flujo 4P en código**: interceptor de precios envía speech con precio+oportunidad+presión+pregunta. msg_precios post-afiche corto (solo 1 hijo + promo + paquetes en 1 línea, sin multi-hijo). (5) **PARTE 3 sin precio**: cierre emocional "¿Te gustaría regalarte un sábado diferente?" sin dar precio. Precio+promo solo en FASE 2B cuando dice sí, afiche automático al detectar "100.000" en respuesta Claude. (6) **Regla de silencio**: si Claude no sabe → "Te respondo en un minuto 😊" + alerta admin WhatsApp+Telegram con link wa.me. (7) **Fix regex fecha reserva**: "Sábado X \| Yh" no matcheaba (solo aceptaba "a las"). Ahora acepta pipe y "a las". Break solo si fecha extraída. (8) **Airtable**: conceptos F.PRUEBA 100MIL y F.PRUEBA 180MIL creados en PRUEBA FENIX. pagos.py: +prueba_promo (100k/150k/180k), +paquete_5 (350k), +paquete_12 (750k). (9) **Fix followup variable indefinida**: `_enviar_afiche_y_followup` referenciaba `followup` sin definir → NameError silencioso en espejo Telegram. Eliminada. (10) **"promo"/"paquete" en detector**: padre que dice "promo" o "paquete" ahora activa interceptor de precios. |
| 2026-05-15 (sesion 2) | **MONTO + CONCEPTO por tipo + promo madre + alertas + fixes (19 commits).** (1) **Campo MONTO en PRUEBA FENIX**: nuevo campo numerico con total pagado. CONCEPTO ya no se deduce del monto sino del tipo de compra: PRUEBA 1HIJO/2HIJOS/3HIJOS, PAQUETE5, PAQUETE12, FENIXMAMA. (2) **Promo madre completa**: PAGO PROMOMADRE checkbox en LEADS FENIX, endpoint auditoria-promomadre (cruza Airtable vs PostgreSQL), endpoint marcar/revertir-pago-promomadre, comando "promo madre" con linea Pagaron + nombre + link Telegram + telefono de cada lead. 7 pagos reales confirmados (3 falsos revertidos). Handler promo madre crea PRUEBA FENIX con concepto FENIXMAMA. (3) **Promo madre DESACTIVADA**: flag _PROMO_MADRE_ACTIVA=False, 3 handlers protegidos con guard, system prompt ya no inyecta promo. Causa: "quiero" + "promo" interceptaba leads que querian otra promo (caso 595994468797 queria 100k, le cobro 350k). (4) **Alertas llamada/silencio**: van a ADMIN_PHONE (595982790407), incluyen link al topic de Telegram del lead. (5) **7 familias FENIXMAMA creadas en Airtable**: FAMILIAS + NINOS + RESERVAS (sab 16/5) + vinculacion LEADS/PRUEBA/PAGOS. Papa/mama en campos correctos segun genero. (6) **Fix msg_precios**: aclara precio por 1 hijo y 2 hijos separado. (7) **Fix formulario post-pago**: pide hijos segun monto pagado, no segun mencion (90/100k=1 hijo, 120/150k=2, 150/180k=3). (8) **Fix "nene/a"**: formulario dice "tu hijo/a". (9) **Datos cargados manualmente**: Marcelo Saucedo/Mauricio (100k, 23/5 + 30/5), correcciones encoding tildes en 6 registros. (10) **Fechas PRUEBA FENIX normalizadas**: todos los formatos ("16 de mayo", "16/5/26", "(por definir)") convertidos a ISO. (11) **Comando "resumen flias"**: nombre hijo + nombre padre + link wa.me por turno, emojis rotativos. (12) **INSCRIPTO checkbox en PRUEBA FENIX**: niños que pasaron a FAMILIAS se marcan INSCRIPTO=true, excluidos del conteo de agendados. 9 registros FENIXMAMA marcados. |
| 2026-05-17 (sesion 2) | **Web alumnos + resumen anuncios mejorado + tabla GASTOS FENIX (12 commits).** (1) **Web pública fenixkidsacademy.com**: endpoints `/api/alumnos` y `/api/reservas` en Railway. Página `alumnos.html` (grid cards con foto+nombre+edad, ficha individual con familia+WhatsApp). Página `reservas.html` (reservas por turno con foto, padre, link WhatsApp). Repo `ivanlafuentepy/fenixkidsacademy-web` creado y conectado a Cloudflare Pages (deploy automático). (2) **Campo LINK WEB** creado en NIÑOS FENIX y PRUEBA FENIX (tipo URL), 47+58 links cargados. (3) **Resumen reservas**: link a página web como header (`fenixkidsacademy.com/reservas?fecha=X`). Links individuales por niño removidos (excedían 4096 chars WhatsApp). (4) **Tabla GASTOS FENIX** creada en Airtable: gasto real de anuncios por día. 15 registros cargados (3-17/5). El resumen lee de esta tabla, fallback 200k/día. (5) **Resumen anuncios mejorado**: usa campo MONTO directo (no solo concepto), desglose por concepto real, FENIXMAMA incluido, solo cuenta monto>0, total separado: Pruebas + Fenixmama + Plan inscriptos → Total recaudado vs Total anuncios → Diferencia. |
| 2026-05-16/17 | **Fix monto afiche + registrar cara mejorado (6 commits).** (1) **Fix monto_prueba_por_hijos 90k→150k**: afiche de precios ("Prueba: 90.000 Gs") matcheaba regex `prueba/eval:` antes del monto real acordado. Fix: skip mensajes-afiche (contienen "1 hijo" + "2 hijos") + nuevos patrones ("X Gs por N sábados", "Pagás al llegar: X Gs"). (2) **Registrar cara — lista numerada**: si hay múltiples matches muestra lista 1-N para seleccionar. Antes decía "usá nombre completo". (3) **Registrar cara — foto+caption en un mensaje**: caption de imagen se pasa como campo en MensajeEntrante. Si mandás foto con caption "registrar cara matheo" procesa todo de una. Si hay múltiples, muestra lista y reutiliza la foto al elegir. (4) **Búsqueda tolerante a acentos**: "cesar" encuentra "César" generando variantes con/sin acentos. (5) **Busca en NIÑOS + PRUEBA simultáneamente**: antes solo en NIÑOS y si matcheaba no iba a PRUEBA (bug "max" → Maximiliano en vez de Max Ki Bae). (6) **Búsqueda multi-palabra**: "max lee" busca registros con "max" Y "lee" en NOMBRE+APELLIDO (ambas tablas). (7) **Foto como attachment**: al registrar cara, sube la imagen al campo FOTO de Airtable via content.airtable.com (base64). Nueva función `subir_attachment_airtable` en airtable_client.py. |
| 2026-05-18 | **Página follow-up pruebas + skills + modo padre (7 commits).** (1) **Página FU pruebas 16/mayo**: consulta Airtable PRUEBA FENIX, filtra PRESENTE=true, excluye INSCRIPTO, agrupa por padre, genera mensajes personalizados por rango etario (3-5/6-8/9-12/mixto). Estructura: pregunta abierta + consejo de profe + promo 12 clases 750k + P.D. sin vencimiento. HTML dark mode con botones wa.me directos, served desde `/fu/{archivo}?key=ADMIN_KEY`. 13 familias, 18 niños. (2) **Endpoint /test-envio/{tel}**: diagnóstico de envío — envía mensaje desde Railway (no curl directo a Meta). Resolvió problema de ventana 24h. (3) **Skills Claude Code**: carpeta `.claude/skills/` con lógica de `/fusabado` y `/endpoint`. Proxies en `commands/`. `/fusabado` automatiza todo el proceso FU (Airtable→HTML→deploy→WhatsApp). `/endpoint` analiza conversaciones de prod. (4) **"modo padre" unificado**: reset completo + activa modo padre en un solo comando. "holayosoyfenix" sigue como alias. (5) **Admin modo secre por defecto**: mensajes no-comando se ignoran (comportamiento correcto, no bug). |
| 2026-05-22 | **Inyección fecha completa + hooks SessionStart/End + CLAUDE.local.md + template aviso horario (2 commits).** (1) **brain.py**: `_contexto_fechas()` ahora inyecta hora actual (`%H:%M`), MAÑANA y PASADO MAÑANA pre-calculados + regla "NUNCA calcules fechas" (patrón Dorita). (2) **Hooks CC**: settings.json con SessionStart (fecha, branch, git status, últimos 5 commits) y SessionEnd (timestamp). Sin TZ (Git Bash Windows). (3) **CLAUDE.local.md**: config local (rutas, notas máquina), agregado a .gitignore. (4) **Template aviso horario**: `scripts/gen_aviso_horario.py` + `static/aviso-cambio-horario.html` commiteados como template reutilizable para avisos masivos (cambio horario por clima, feriado, etc). (5) **Análisis prompt curso IA**: comparación completa del prompt maestro del curso vs sistema FENIX. Conclusión: FENIX más avanzado, pero los .md locales del curso son correctos para enseñar (visibilidad, control, valor pedagógico). |
| 2026-05-23 | **Migración a Tool Use + FSM — Fases 0-3 (5 commits).** Diagnóstico: caso Marcelo Saucedo (reagendamiento no se reflejó en Airtable porque regex dependía de cómo Claude redactaba). Investigación de 7 arquitecturas de agentes. Decisión: Tool Use + FSM híbrido. **Fase 0**: 5 sets in-memory (`_afiche_enviado`, `_prueba_creada`, etc.) migrados a columna `estado_json` (JSON) en ConversacionAB (PostgreSQL). Ya no se pierden en redeploy. **Fase 1**: 10 funciones `_padre_pregunta_*` extraídas a `agent/tools/detectores.py`. main.py -110 líneas. **Fase 2**: `brain.py` con soporte tool_use (retrocompatible). Tool use loop con max 3 rounds. Archivos nuevos: `tool_definitions.py`, `tool_executor.py`, `tools/info.py`, `tools/reservas.py`. **Fase 3**: Feature flag `USE_TOOL_USE` conecta tools al flujo principal. Default false. Solo 2 tools (reagendar_clase + confirmar_reserva) — FAQ se queda como interceptores regex (gratis). Decisión: Tool Use solo para acciones que fallan con regex, no para FAQ. Documento completo en Vault: `FENIX KIDS/2026-05-23 Migracion Tool Use + FSM.md`. |
| 2026-05-24 | **Auditoría completa + council 5 advisors + fix 3 bugs críticos (1 commit).** (1) **Auditoría total**: lectura exhaustiva de los 26 archivos Python (14,393 líneas), análisis completo de main.py (8,010 líneas — 34 pasos del webhook, 25 rutas, 6 background loops, 10 interceptores, estado en memoria). Referencia: guía Anthropic Certified Architect. Diagnóstico con prioridades C1-C3 (crítico), A1-A4 (alto), M1-M4 (medio). (2) **Council de 5 advisors** (metodología Karpathy): Contrarian, First Principles, Expansionist, Outsider, Executor — con peer review anónimo y síntesis del Chairman. Veredicto: "la dirección es correcta, la secuencia no". Secuencia corregida: fix bugs → tools con hooks en brain actual → partir monolito. (3) **MIGRACION TOTAL.md**: documento de 700+ líneas con estado actual detallado, diagnóstico, plan de reconstrucción (50 archivos), council completo, decisión final, registro de implementación. (4) **Fix confirmar_reserva** (BOMBA desactivada): executor implementado en reservas.py con parseo de fecha multi-formato + validación sábado + update Airtable + notificación admin. (5) **escalar_a_humano como tool**: reemplaza frase mágica "Te respondo en un minuto". Claude llama tool → handoff estructurado a Telegram/WhatsApp (motivo, resumen, link WA, link topic) → silencia agente. 5 motivos enum. (6) **Errores estructurados**: tool_executor reescrito con categorías transient/validation/business/internal + is_retryable. brain.py envía is_error:true en tool_results. (7) **Descripciones tools mejoradas**: qué retorna, cuándo usar, cuándo NO usar (best practice Anthropic). main.py NO tocado. |
