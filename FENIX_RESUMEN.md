...
| 2026-05-26 | **Fix conversacional + sistema agent-skills (3 commits).** (1) **Fix detector hermanos**: keyword "familia" sacado de `padre_pregunta_hermanos` — "En familia iremos" (padre preguntando si van mamá+papá) disparaba afiche hermanos incorrectamente. (2) **Nombre del lugar**: "LA MANSIÓN LAFUENTE" → "LA CASONA LAFUENTE" en prompt. "Mansión" solo como descripción ("una mansión de 3000m2"). (3) **Tono amable**: nueva regla PROHIBIDO — siempre "por favor" al pedir datos, nunca tono imperativo seco. (4) **Sistema agent-skills implementado** (basado en addyosmani/agent-skills, 45k stars): meta-skill router en `.claude/skills/using-fenix-skills/SKILL.md` se inyecta automáticamente al inicio de cada sesión via SessionStart hook. 4 skills nuevos: `/pre-cambio` (verificación antes de editar código crítico), `/pre-deploy` (verificación antes de git push), `/debug` (workflow debugging producción), `/verificar` (doubt-driven development). Anti-racionalizaciones calibradas con errores reales del proyecto. Documentación completa en `agent-skills-implementacion.md` + Obsidian. |
